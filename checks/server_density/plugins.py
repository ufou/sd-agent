...
class Plugins(Check):
    ...
    def check(self, agentConfig):
        ...
        self.log.debug('getPlugins: start')
        ...
        if plugin_directory:
            ...
            self.log.debug('getPlugins: plugin_directory not set')
        ...
        if self.plugins is None:
            ...
            for root, dirs, files in os.walk(plugin_directory):
                for name in files:
                    self.log.debug('getPlugins: considering: %s', name)
            ...
            for plugin_name in plugins:
                self.log.debug('getPlugins: loading %s', plugin_name)
                ...
                try:
                    ...
                    self.log.debug('getPlugins: imported %s', plugin_name)
                    ...
                    try:
                        plugin_obj = plugin_class(
                            agentConfig, self.log, self.raw_config)
                    except TypeError:

                        try:
                            plugin_obj = plugin_class(
                                agentConfig, self.log)
                        except TypeError:
                            # Support older plugins.
                            plugin_obj = plugin_class()

                    self.log.debug('getPlugins: instantiated %s', plugin_name)
        ...
        if self.plugins is not None:
            self.log.debug('getPlugins: executing plugins')
            ...
            self.log.debug('getPlugins: returning')
            # Each plugin should output a dictionary so we can convert it to
            # JSON later
            return output

        else:
            self.log.debug('getPlugins: no plugins, returning false')
