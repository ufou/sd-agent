...
class AgentMetrics(AgentCheck):
    ...
    def _register_psutil_metrics(self, stats, names_to_metric_types):
        ...
        :param stats: a dictionary that looks like:
        {
         ...
         This creates a metric like `serverdensity.agent.collector.{key_1}.{key_2}` where key_1 is a top-level
         key in `stats`, and key_2 is a nested key.
         E.g. serverdensity.agent.collector.memory_info.rss
        """
        ...
        for k, v in stats.iteritems():
            ...
            if isinstance(v, dict):
                ...
                full_metric_name = 'serverdensity.agent.collector.{0}'.format(k)
